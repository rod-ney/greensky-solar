  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        ...
        {tasks.length === 0 ? (
          <div className="border-t border-slate-100 px-5 py-10 sm:p-12">
            <WorkspaceEmpty
              icon={ClipboardList}
              title="No tasks yet"
              description="Add milestones with owners and due dates to track execution, power the timeline, Gantt chart, and project progress."
            >
              <Button
                icon={Plus}
                size="sm"
            ...
            {tasksGroupedByDueDay.length === 0 ? (
              <div className="px-5 pb-10 pt-2">
                <WorkspaceEmpty
                  variant="compact"
                  icon={ClipboardList}
                  title={tasks.length === 0 ? "No tasks yet" : "No tasks in this filter"}
                  description={
                    tasks.length === 0
                      ? "Use Add task above to create your first milestone."
                      : "Change the status chips above or choose “All” to see every task."
                  }
                />
              </div>
            ) : (
            ...
            {filteredTasks.length === 0 ? (
              <div className="p-8">
                <WorkspaceEmpty
                  variant="compact"
                  icon={ChartGantt}
                  title={tasks.length === 0 ? "Nothing to plot yet" : "No tasks for this filter"}
                  description={
                    tasks.length === 0
                      ? "Add tasks with due dates and the Gantt chart will map them across your project window."
                      : "Clear the status filter to see all tasks on the timeline."
                  }
                />
              </div>
            ) : (
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        {tasks.length === 0 ? (
          <WorkspaceEmpty
            ...
            description="Add tasks in the task manager—the daily timeline will summarize what’s scheduled on each shift."
          />
        ) : tasksGroupedByDueDay.length === 0 ? (
          <WorkspaceEmpty
            variant="compact"
            ...
            description="Adjust the status filter in Tasks to match items you want on the timeline preview."
          />
        ) : (
